...
{
    public class ElectricalEngineeringCapacitorSolver : SolverINPC
    {
        public ElectricalEngineeringCapacitorSolver()
        {
            // Calculates up to 3 Capacitors in series (s1, s2, s3) or in parallel (p1, p2, p3)            // If you swap series for parallel, you get a 
            Equations.Add(new Equation("Series", new List<string>() { "S1", "S2", "S3" }, () => { return  1.0 / ((1.0 / S1) + (1.0 / S2) + (1.0 / S3));  }));
            Equations.Add(new Equation("Parallel", new List<string>() { "P1", "P2", "P3" }, () => { return P1 + P2 + P3; }));

            InitEquivLists();
        }


        private double _S1 = Double.MaxValue;
        public double S1 { get { return _S1; } set { if (value == _S1) return; _S1 = value; OnPropertyChanged("S1"); } }

        private double _S2 = Double.MaxValue;
        public double S2 { get { return _S2; } set { if (value == _S2) return; _S2 = value; OnPropertyChanged("S2"); } }

        private double _S3 = Double.MaxValue;
