...
{
    public class RTLSensorLight : IObjectValue
    {
        public string PreferredName { get { return "Light"; } }
        ...
        {
            var arglist = new List<IExpression>() { new NumericConstant(args.Reading.IlluminanceInLux) };
        ...
        public void Start(BCRunContext context, string fname)
        {
            ...
            if (sensor == null)
            {
                RTLSystemX.AddError($"Light: this system has no light sensor");
            }
            else
            {
